...
import { getOffers, getOffersError, getSubscriptions, getSubscriptionsError, State } from '../../state';
...
export class UserDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.offers$ = this.store.select(getOffers).pipe(
      tap((data) => data.length ? this.offersLoading = false : this.offersLoading = true));
    this.offersError$ = this.store.select(getOffersError);
  }

  getSubscriptions(offerId: number): void {
    this.store.dispatch(UserPageActions.loadSubscriptions({offerId}));
    this.subscriptions$ = this.store.select(getSubscriptions).pipe(
      tap((data) => data.length ? this.subscriptionsLoading = false : this.subscriptionsLoading = true),
      tap((data) => console.log(data)));
    this.subscriptionsError$ = this.store.select(getSubscriptionsError);
  }
