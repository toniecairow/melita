...
export class UserEffects {
  ...
  loadOffers$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      exhaustMap(() =>
        this.userService.getOffers().pipe(
  ...
  loadSubscriptions$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.userService.getSubscriptions(action?.offerId).pipe(
          map((subscriptions) =>
            UserApiActions.loadSubscriptionsSuccess({ subscriptions })
          ),
          // catchError(({error}) => of(UserApiActions.loadSubscriptionsFailure({ ...error })))
          catchError((error) => {
            return of(
              UserApiActions.loadSubscriptionsFailure({
                error: "No Offer subscriptions found",
              })
            );
          })
        )
    ...
    return this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.userService.logout().pipe(
          ...
          catchError(({ error }) =>
            of(UserApiActions.logoutUserFailure({ ...error }))
          )
        )
      )
    );
