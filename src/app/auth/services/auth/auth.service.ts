...
export class AuthService {
  ...
  static getControlError(formGroup: FormGroup, formControlName: string, error: string) {
    if(!formGroup && !formControlName && error) {
      return true;
    }
    return formGroup.get(formControlName)?.invalid && formGroup.get(formControlName)?.errors && formGroup.get(formControlName)?.errors?.[error]
  }
  ...
  login({ username, password }: UserCredentials): Observable<any>{
    ...
    return this.http.post<any>(`${this.backendUrl}/login`,{username, password}, { params, headers }).pipe(
      tap(({authToken}) => localStorage.setItem('tokenData', JSON.stringify({authToken}) )),
